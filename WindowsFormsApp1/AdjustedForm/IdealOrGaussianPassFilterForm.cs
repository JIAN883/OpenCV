...
namespace WindowsFormsApp1.AdjustedForm
{
    public partial class IdealOrGaussianPassFilterForm : Form
    {
        ...
        private void IdealOrGaussianPassFilterForm_Load(object sender, EventArgs e)
        {
            checkBoxIdeal.Checked = true;
        ...
        {
            checkBoxGaussian.Checked = !checkBoxIdeal.Checked;
            ImageProssce(false);
        }

        private void checkBox2_CheckedChanged(object sender, EventArgs e)
        {
            checkBoxIdeal.Checked = !checkBoxGaussian.Checked;
            ImageProssce(false);
        }
        ...
        {
            bool isIdeal = false, isHighPass = false;
            int value = (int)AdjustedFormManager.GetTrackValue(trackBar1.Maximum, trackBar1.Value, max, min);
            Mat src = source.Clone();
            if (checkBoxIdeal.Checked)
                isIdeal = true;
            if (checkBoxHighPass.Checked)
                isHighPass = true;
            
            idealOrGaussianPassFilter(src.Data, src.Width, src.Height, isIdeal, isHighPass, value, isAddOri, out IntPtr dst);
